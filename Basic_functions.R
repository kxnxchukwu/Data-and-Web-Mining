...
# Going to use the str() command lots - stands for 'structure'

# Vectors
x = c(2.3, -4.6, 5.7) # the c() stands for 'collection'
...
prostate2 = read.table('prostate.csv', header=TRUE, sep=',',
                       nrows = 50,
                       skip = 10)
str(prostate2)
...
# For my plot I thought it might be interesting to show density of
# independent variable against the dependent log PSA.  I think this
# is a little more obvious in communicating the general shape of the
# relationship.

# Extract just the training data
dat<-prostate[which(prostate$train == TRUE),][1:9]

# Use ggplot for nicer-looking plots - you will need to install it if not already installed
library(ggplot2)
...
require(gridExtra) # again you will need to install this package if not already installed.
...
# Comments:
# Gleason level 9 is highly concentrated between ~2 and 4 l-PSA. Level
# 6 appears concentrated in the lower-half of the l-PSA range whereas
# level 7 appears concentrated in the upper-half of the range. Gleason
# level 8 seems to appear on a single observation.

# The Age factor plot indicates that l-PSA seems to be more concentrated
# in a narrower range of l-PSA as the corresponding age group gets older.
# The oldest group appears to be concentrated to the upper range of l-PSA.
